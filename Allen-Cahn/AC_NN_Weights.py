...
weight_layer_sizes = [2, 20, 20, 1]

weight_model_init = neural_net(weight_layer_sizes)
weight_model_col = neural_net(weight_layer_sizes)
...
def loss(x_f_batch, t_f_batch,
    ...
    u_pred = weight_model_init(tf.concat([x_lb, t_lb],1))
    u_x_pred = weight_model_init(tf.concat([x_lb, t_lb],1))
    u0_weights = weight_model_init(tf.concat([x0, t0],1))
    col_weights = weight_model_col(tf.concat([x_f_batch, t_f_batch], 1))
    # tf.print(tf.reduce_mean(u0_weights))

    u_lb_pred, u_x_lb_pred, = u_x_model(x_lb, t_lb)
    u_ub_pred, u_x_ub_pred, = u_x_model(x_ub, t_ub)

    mse_0_u = tf.reduce_mean(tf.square(tf.abs(u0_weights)*(u0 - u0_pred)))
    mse_b_u = tf.reduce_mean(tf.square(u_pred*tf.math.subtract(u_lb_pred,u_ub_pred))) + \
            tf.reduce_mean(tf.square(u_x_pred*tf.math.subtract(u_x_lb_pred, u_x_ub_pred)))

    mse_f_u = tf.reduce_mean(tf.square(tf.abs(col_weights)*f_u_pred))
...
def grad(model, x_f_batch, t_f_batch, x0_batch, t0_batch, u0_batch, x_lb, t_lb, x_ub, t_ub):
    with tf.GradientTape(persistent=True) as tape:
        loss_value, mse_0, mse_b, mse_f = loss(x_f_batch, t_f_batch, x0_batch, t0_batch, u0_batch, x_lb, t_lb, x_ub, t_ub)
        grads = tape.gradient(loss_value, u_model.trainable_variables)
        #print(grads)

        grads_weights_init = tape.gradient(loss_value, weight_model_init.trainable_variables)
        grads_weights_col = tape.gradient(loss_value, weight_model_col.trainable_variables)

    return mse_f, mse_b, mse_0, grads, grads_weights_init, grads_weights_col, loss_value
...
def fit(x_f, t_f, x0, t0, u0, x_lb, t_lb, x_ub, t_ub, tf_iter, newton_iter, n_init):
    ...
    for epoch in range(tf_iter):
        ...
        for i in range(n_batches):
            ...
            loss_c, loss_b, loss_i, grads, grads_weights_init, grads_weights_col, total_loss = grad(u_model, x_f_batch, t_f_batch, x0_batch, t0_batch, \
                                                                      u0_batch, x_lb, t_lb, x_ub, t_ub)
            tf_optimizer.apply_gradients(zip(grads, u_model.trainable_variables))
            grads_weights_init = [-l for l in grads_weights_init]
            grads_weights_col = [-l for l in grads_weights_col]
            tf_optimizer_weights.apply_gradients(zip(grads_weights_init, weight_model_init.trainable_variables))
            tf_optimizer_weights.apply_gradients(zip(grads_weights_col, weight_model_col.trainable_variables))

        if epoch % 100 == 0:
            elapsed = time.time() - start_time
            print('It: %d, Time: %.2f' % (epoch, elapsed))
            tf.print(f"mse_0: {loss_i}  mse_b:  {loss_b}  mse_f: {loss_c} loss_total: {total_loss}")
            tf.print(tf.reduce_mean(weight_model_init(tf.concat([x0, t0],1))), tf.reduce_mean(weight_model_col(tf.concat([x_f_batch, t_f_batch],1))))
            start_time = time.time()
