...
weight_layer_sizes = [2, 20, 20, 20, 20, 3]

weight_model = neural_net(weight_layer_sizes)
...
def loss_init(x0, t0, u0):
    u0_pred = u_model(tf.concat([x0, t0],1))[0]
    weight_model_pred_init = weight_model(tf.concat([x0, t0],1))
    # mse_0_u = tf.reduce_mean(tf.square(weight_model_pred_init*(u0 - u0_pred)))
    mse_0_u = tf.reduce_mean(tf.square(100*(u0 - u0_pred)))

    return  mse_0_u, tf.reduce_mean(tf.square((u0 - u0_pred)))

def loss_col(x_f_batch, t_f_batch):

    f_u_pred, col_weights = f_model(x_f_batch, t_f_batch)
    weight_model_pred_col = weight_model(tf.concat([x_f_batch, t_f_batch], 1))

    # mse_f_u = tf.reduce_mean(tf.square(weight_model_pred_col*f_u_pred))
    mse_f_u = tf.reduce_mean(tf.square(f_u_pred))

    return  mse_f_u, tf.reduce_mean(tf.square(f_u_pred))

def loss_bound(x_lb, t_lb, x_ub, t_ub):
    u_lb_pred, u_x_lb_pred, = u_x_model(x_lb, t_lb)
    u_ub_pred, u_x_ub_pred, = u_x_model(x_ub, t_ub)
...
def f_model(x,t):
    ...
    return f_u, col_weights

@tf.function
def u_x_model(x, t):
    u = u_model(tf.concat([x, t],1))[0]
    u_x = tf.gradients(u, x)

    return u, u_x

@tf.function
def grad(model, x_f_batch, t_f_batch, x0_batch, t0_batch, u0_batch, x_lb, t_lb, x_ub, t_ub):
    with tf.GradientTape(persistent=True) as tape:
        loss_c, mse_f = loss_col(x_f_batch, t_f_batch)
        loss_i, mse_0 = loss_init(x0_batch, t0_batch, u0_batch)
        loss_b = loss_bound(x_lb, t_lb, x_ub, t_ub)
        loss_value, _, _, _ = loss(x_f_batch, t_f_batch, x0_batch, t0_batch, u0_batch, x_lb, t_lb, x_ub, t_ub)
        grads_col = tape.gradient(loss_c, u_model.trainable_variables)
        grads_i = tape.gradient(loss_i, u_model.trainable_variables)
        grads_b = tape.gradient(loss_b, u_model.trainable_variables)
        grads = tape.gradient(loss_value, u_model.trainable_variables)
        #print(grads)

        #grads_weights = tape.gradient(loss_value, weight_model.trainable_variables)

    return mse_f, loss_b, mse_0, grads_col, grads_i, grads_b, grads


def fit(x_f, t_f, x0, t0, u0, x_lb, t_lb, x_ub, t_ub, tf_iter, newton_iter, n_init):
    ...
    tf_optimizer_c = tf.keras.optimizers.Adam(lr = 0.005, beta_1=.99)
    tf_optimizer = tf.keras.optimizers.Adam(lr = 0.005, beta_1=.99)
    tf_optimizer_b = tf.keras.optimizers.Adam(lr = 0.005, beta_1=.99)
    ...
    print("Training boundary conditions")
    # for i in range(n_init):
    #     loss_c, loss_b, loss_i, grads_c, grads_i, grads_b, _ = grad(u_model, x_f_batch, t_f_batch, x0_batch, t0_batch, \
    #                                                                 u0_batch, x_lb, t_lb, x_ub, t_ub)
    #
    #     # tf_optimizer_c.apply_gradients(zip(grads_b, u_model.trainable_variables))
    #
    #     # _, _, loss_i, _, grads_i, _  = grad(u_model, x_f_batch, t_f_batch, x0_batch, t0_batch, \
    #     #                                                           u0_batch, x_lb, t_lb, x_ub, t_ub)
    #     tf_optimizer_c.apply_gradients(zip(grads_i, u_model.trainable_variables))
    #     if i % 100 == 0:
    #         elapsed = time.time() - start_time
    #         print('It: %d, Time: %.2f' % (i, elapsed))
    #         tf.print(f"mse_0: {loss_i}  mse_b  {loss_b}  mse_f: {loss_c}")
    #         start_time = time.time()
    ...
    for epoch in range(tf_iter):
        ...
        for i in range(n_batches):
            ...
            loss_c, loss_b, loss_i, grads_c, grads_i, grads_b, grads  = grad(u_model, x_f_batch, t_f_batch, x0_batch, t0_batch, \
                                                                      u0_batch, x_lb, t_lb, x_ub, t_ub)
            # update

            # tf_optimizer_c.apply_gradients(zip(grads_c, u_model.trainable_variables))
            # if epoch % 10 == 0:
            #     _, _, _, _, _, _, grads = grad(u_model, x_f_batch, t_f_batch, x0_batch, t0_batch, \
            #                                                               u0_batch, x_lb, t_lb, x_ub, t_ub)
                # grads_weights = [-l for l in grads_weights]
            tf_optimizer_c.apply_gradients(zip(grads, u_model.trainable_variables))
...
def get_loss_and_flat_grad(x_f_batch, t_f_batch, x0_batch, t0_batch, u0_batch, x_lb, t_lb, x_ub, t_ub):
...
#train loop
fit(x_f, t_f, x0, t0, u0, x_lb, t_lb, x_ub, t_ub, tf_iter = 10000, newton_iter = 1, n_init=2000)
